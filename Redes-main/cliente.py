...
def sendWRQ(id, udpcsocket, blockn, sap, bfsz, fileName, net_mode, user):
    while True:
        try:
            wrqpkg = (2).to_bytes(2,'little') + fileName.encode() + (0).to_bytes(1,'little') + net_mode.encode() + (0).to_bytes(1,'little') + user.encode()
...
def sendRRQ(udpcsocket, sap, fileName, net_mode, user):
    rrqpkg = (1).to_bytes(2,'little') + fileName.encode() + (0).to_bytes(1,'little') + net_mode.encode() + (0).to_bytes(1,'little') + user.encode()
...
class Cliente(Thread):
    ...
    def run(self): #Metodo que se ejecutara con la llamada start
        ...
        if modo == 'wrq':
            ...
            port = sendWRQ(self.id, UDPClientSocket, block, serverAddressPort, bufferSize, nombre_archivo, netmode, user)
            block += 1
        ...
        if modo == 'rrq':
            ...
            sendRRQ(UDPClientSocket, serverAddressPort, nombre_archivo, netmode, user)
            ...
            while True:
                ...
                try:
                    ...
                    if finalBlock:
                        ...
                        print(bcolors.OKGREEN + "RRQ SIN RESPUESTA - Fin de Conexion! (Puede que haya ingresado mal las credenciales)" + bcolors.ENDC)
                    break
