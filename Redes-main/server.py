...
#print(verificaUsuario("127.0.0.1"))
...
while(True):
    ...
    if True: #?
        ...
        if opCode == 1 :
            ...
            restoParametros= clientMsg[index+1:len(clientMsg)]
            indice = restoParametros.find(b'\x00',2)
            net_mode = restoParametros[0:indice].decode()
            user = restoParametros[indice:].decode()[1:]
           
            verificacion = verificaUsuario(user)
...
if(verificacion):
    ...
    print(bcolors.FAIL + "CREDENCIALES INVALIDAS!" + bcolors.ENDC)
